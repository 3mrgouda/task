    ...
    return (
        <div className=' w-full xl:mx-auto ' >
            ...
            <div className='  flex xl:w-11/12 flex-col w-full xl:flex-row xl:items-center xl:justify-between mx-auto gap-2'>
                <div  className="logo flex items-center justify-between text-4xl sm:text-5xl"><Link to={'../../'} className="flex items-center">Begin <img src={LogoIcon} className='inline-block w-[50px]' alt="" /><span className='font-bold'>Front</span></Link> <div className='max-sm:block hidden w-20 '></div>
        ...
        <img src={HeroImg} className='mt-[89px] xl:w-full xl:h-[670px] xl:object-cover fixed -z-10 top-0 ' alt="" />
            ...
            <div className=' flex items-center xl:w-11/12  justify-center  mx-auto mt-[330px] md:mt-[600px] xl:mt-[800px]'>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 ">
                    {
                        api.map((item, i) => (
                            <div key={i} className="card my-5 mx-2  bg-white border-solid border-gray-100 border-2   rounded-xl p-5 shadow-lg  ">
                                <div className="flex flex-col gap-4">
                                  ...
                                  <button className='bg-[#4B6BFB] bg-opacity-5 px-4 text-[#4B6BFB] rounded-md w-auto mx-auto capitalize  text-nowrap flex items-center'><a className='' href={item.toolSite}>{item.toolName}</a> <GoLinkExternal/> </button>  
                                    ...
                                    <Link to={`${i}`} onClick={()=>handleDynamic(i)}  className='text-white rounded-md py-2 text-center   px-4 bg-[#4B6BFB] hover:bg-opacity-5 hover:text-[#4b6bfb] duration-200  capitalize text-xl text-nowrap'>More Details</Link>
                                 </div>
                            </div>
                        ))}
